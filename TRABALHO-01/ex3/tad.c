...
struct no
{
    int info;      // Informação que será guardada pelo no
...
{
    int i = 1;          // Começa a verificar partindo do elemento 1 (primeiro)
    ...
    {                    // Se nao chegou na pos desejada e o prox elemento é valido: avançar
...
int listas_iguais(Lista *lista_1, Lista *lista_2)
{
    ...
    while (((aux_1)->prox != NULL && (aux_2)->prox != NULL) && ((aux_1)->info == (aux_2)->info))
    {
        (aux_1) = (aux_1)->prox;
        (aux_2) = (aux_2)->prox;
    }
...
int media(Lista *lista)
{
    ...
    return value / tamanho_lista(lista);
    return 1;
}
...
int intercalar_listas(Lista *lista_1, Lista *lista_2, Lista *lista_3)
{
    ...
    for (int i = 0; i < tamanho_lista(lista_1); i++)
    {
    ...
    for (int i = 0; i < tamanho_lista(lista_2); i++)
    {
